...
import { getDbProducts, updateDbProduct } from "@/lib/db";
import { LayoutDashboard, ShoppingCart, Percent, Save, ArrowLeft, Plus } from "lucide-react";
...
export default function AdminEditProduct({ params }) {
  ...
  useEffect(() => {
    ...
    async function loadProductData() {
      try {
        ...
        // Find match
        const match = list.find(p => p.id === productId || p._id === productId);
        if (match) {
          setNewProd({
            id: match.id || match.slug || match._id,
            name: match.name || "",
            category: match.category || "Premium Abayas",
            price: match.price || "",
            fabric: match.fabric || "",
            image: match.image || "",
          ...
          if (Array.isArray(match.sizes)) {
            setSelectedSizes(match.sizes);
          }
          
          if (Array.isArray(match.details)) {
            const lengthDetail = match.details.find(d => d.toLowerCase().includes("length"));
            if (lengthDetail) {
              const num = lengthDetail.replace(/[^0-9]/g, "");
              if (num) setDefaultLength(num);
            }
          }
        }
      } catch (err) {
  ...
  const handleEditSubmit = async (e) => {
    ...
    // Construct details array to keep format
    const details = [
      "Material: Luxury selection",
      "Included: Sheila set",
      "Care: Professional clean",
      `Length: ${defaultLength} inches`
    ];
    ...
    try {
      await updateDbProduct(productId, updatedData);
  ...
  if (loading) {
    return (
      <div className="min-h-screen bg-bg-deep text-primary-text font-sans flex items-center justify-center">
        <p className="font-serif text-sm tracking-widest text-primary-gold animate-pulse uppercase">Loading Silhouette Details...</p>
      </div>
    );
  }
        ...
        <div className="border-b border-border-custom pb-6 mb-10 flex flex-col md:flex-row md:items-center md:justify-between gap-4">
          <div>
            ...
            <p className="text-[10px] text-muted-text uppercase tracking-widest mt-1">
              System Administration / Edit Product
            </p>
          </div>
            ...
            <div className="space-y-6">
              <div className="flex items-center gap-2 pb-3 border-b border-border-custom">
                ...
                <h3 className="font-serif text-lg font-semibold tracking-wider">
                  Edit Lookbook Product
                </h3>
              </div>

              {/* Edit Product Form */}
              <form onSubmit={handleEditSubmit} className="p-5 border border-border-custom bg-bg-secondary/25 grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div className="space-y-1">
                  <label className="text-[9px] uppercase tracking-widest font-bold">Product ID (Slug - Read Only)</label>
                  <input type="text" readOnly disabled value={newProd.id} className="w-full bg-bg-card/50 border border-border-custom p-2 text-xs text-muted-text cursor-not-allowed outline-none" />
                </div>
                <div className="space-y-1">
                ...
                <div className="space-y-1">
                  <label className="text-[9px] uppercase tracking-widest font-bold">Stock Quantity</label>
                  <input type="number" min="0" value={newProd.stock} onChange={(e) => setNewProd({ ...newProd, stock: e.target.value })} className="w-full bg-bg-card border border-border-custom p-2.5 text-xs focus:outline-none focus:border-soft-gold" />
                </div>

                <div className="sm:col-span-2 space-y-1">
                  <label className="text-[9px] uppercase tracking-widest font-bold">Description</label>
                  <textarea rows={3} placeholder="Describe the silhouette profile..." value={newProd.description} onChange={(e) => setNewProd({ ...newProd, description: e.target.value })} className="w-full bg-bg-card border border-border-custom p-3 text-xs focus:outline-none focus:border-soft-gold text-primary-text" />
                </div>
